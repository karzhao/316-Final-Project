...
export default function EditAccountScreen() {
    ...
    const [avatarDataUrl, setAvatarDataUrl] = useState(auth.user?.avatar || "");
    ...
    useEffect(() => {
        ...
        setAvatarDataUrl(auth.user?.avatar || "");
    }, [auth.user]);
    ...
    const passwordProvided = values.password.length > 0 || values.passwordVerify.length > 0;
    const email = auth.user?.email || "";

    const userNameValid = values.userName.trim().length > 0;
    const passwordLongEnough = !passwordProvided || values.password.length >= 8;
    const passwordsMatch = !passwordProvided || values.password === values.passwordVerify;
    const avatarValid = Boolean(avatarDataUrl) && !avatarError;

    const formValid = userNameValid && passwordLongEnough && passwordsMatch && avatarValid;
    ...
    const fieldHelperText = {
        ...
        password: touched.password && !passwordLongEnough ? "At least 8 characters if changing password." : " ",
            ...
            <Container component="main" maxWidth="md" className="auth-card">
                ...
                >
                    ...
                    <Typography variant="body2" className="auth-subtitle">
                        Update your display name, avatar (exactly {REQUIRED_AVATAR_SIZE}x{REQUIRED_AVATAR_SIZE}px), or set a new password.
                    </Typography>
